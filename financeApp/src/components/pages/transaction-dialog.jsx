import React, {useContext, useState, useLayoutEffect, useReducer, useEffect} from "react";
...
export function AddTransaction() {
	...
	useEffect(() => {
		setLoading(true);
		if(userData && userData.accounts.length > 0 && userData.categories.length > 0) {
			setTransactionData({
				userId: userId,
	...
	async function submitTransaction() {
		...
		try{
			if (userId && token && transactionData.categoryId !== "" && transactionData.accountId !== "" && transactionData.amount !== 0) {
				const response = await axios.post(apiUrl+"/api/transaction/create", transactionData, config)
				...
				toast("Event has been created", {
					description: `Your ${response.data.data.type} was ${response.data.data.amount}$ on ${response.data.data.date.split('T')[0]}`,
					action: {
						label: "Undo",
					...
					<div className="flex flex-row justify-start items-start mt-10">
						<div className="flex flex-col justify-start items-start">
							<div className="flex items-center m-2 ml-1">
								<RadioGroup name="type" defaultValue="expense" className="flex flex-row items-center">
