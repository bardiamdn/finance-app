  ...
  async function addNewAccount() {
    ...
    try {
      if (token && userId && newTitle && selectedColor) {
        await axios
          .put(apiUrl + "/api/profile/add-account/" + userId, data, config)
          .then((response) => {
    ...
    <>
      ...
      <AccordionItem value="item-1">
        ...
        <AccordionContent>
          {userData && userData.accounts
            ? userData.accounts.map((item, index) =>
                editClicked ? (
                  <div
                    key={index}
                    className="flex justify-between items-center h-50 ml-2"
                  ...
                  >
                            ...
                            {myColors ? (
                              myColors.map((color, index) => (
                                <DropdownMenuItem
                                  key={color.value}
                                  onClick={() =>
                                    colorChange(item._id, color.value)
                                  }
                                >
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          <AlertDialogTitle>
                            Are you sure you want to delete {item.accountTitle}{" "}
                            and it's transactions?
                          </AlertDialogTitle>
                          <AlertDialogDescription>
                            This action cannot be undone. This will permanently
                            delete this account and related transcations.
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                  ...
                  >
                    ...
                    <div
                      color={item.accountColor}
                      className={`w-1.5 h-7 rounded-md colorDiv`}
                      style={{ backgroundColor: `${item.accountColor}` }}
                    ></div>
          ...
          {addClicked ? (
            <div
              className="flex justify-end items-center mt"
              style={{ width: "95%", height: "50px" }}
            >
              <DropdownMenu className="colorDiv">
                <DropdownMenuTrigger>
          ...
          {editClicked ? (
            <div className="flex justify-end items-center mt-10">
              <Button variant="outline" onClick={editAccount}>
                {" "}
                Save
              </Button>
            </div>
            ...
            <div className="flex justify-end items-center mt-10">
              <Button
                variant="ghost"
                disabled={userData.accounts && userData.accounts.length === 0}
