...
import { TotalBalance, IncomeExpenseBalance, Last90Days, AccountsAnalyticPie } from "./components/pages/top-analytic";
...
const home = () => {
    ...
    async function getProfileData(userId) {
        try {
            if(token && userId) {
                const response = await axios.get(apiUrl+`/api/profile/read-profile/${userId}`, config)
    ...
    async function getBalances() {
        try {
            const response = await axios.get(apiUrl+'/api/balance/timeframe-total-balance/'+userId, config);
    ...
    return (
        <>
        <DataContext.Provider value={{ apiUrl, update, setUpdate, config, userData, setUserData, token, userId, balanceData, getBalances, getProfileData}}>
            <div className="main">
